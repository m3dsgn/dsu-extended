...
class StorageManager(
    ...
    object Constants {
        const val WORKSPACE_FOLDER = "workspace_dsuextended"
    }
    ...
    fun writeZipToUri(entries: Map<String, String>, uri: Uri): String {
        val outputStream = appContext.contentResolver.openOutputStream(uri)!!
        ZipOutputStream(outputStream).use { zipOutput ->
            entries.forEach { (entryName, entryContent) ->
                val zipEntry = ZipEntry(entryName)
                zipOutput.putNextEntry(zipEntry)
                zipOutput.write(entryContent.toByteArray(Charsets.UTF_8))
